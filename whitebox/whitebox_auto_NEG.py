...
case for every branch.  Coverage is a random search, not a construction: what
makes the result valuable is that it is complete and replayable.

The generator carries its own trial bound (WB_TRIALS) and writes where it is told
(WB_LOG), so this runs it once, waits, and parses the finished log.  Anything the
trials failed to reach is reported rather than looped on forever.
...
class FileInfo(object):
    def __init__(self, fieldType, curveType, genus, basis="neg"):
        # Only the split model has a reduced-basis subdirectory; ../g2/ramifiedModel/
        # has no negReduced/ under it.
        #
        # The two bases are not interchangeable: a case records the result the
        # formulas produced, and the bases normalise v against different polynomials
        # at infinity, so a negReduced case replayed against posReduced formulas
        # compares a different divisor.  `whitebox.py` keys on the basis for the same
        # reason.  Genus 3 ships negReduced only, so `basis` is exercised at genus 2
        # only, where three of the fifteen testers are posReduced.
        self.BASIS = basis
        if curveType == "split":
