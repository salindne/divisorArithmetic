...
assume $h = 0$ whenever char($k) \neq 2$, and additionally assume $f_4 = 0$
whenever the characteristic is also not five.
...
made. Following~\cite{Lange_explicit_2005}, $h$ is taken to be non-constant,
since a curve with $h$ constant is supersingular and the discrete logarithm
problem on it is weaker than on an arbitrary curve. Attention is restricted
further to $\deg h = 2$, which the transformation below requires. When $\deg h =
2$ the
following isomorphism transforms a curve to one for which several coefficients
of $h$ and $f$ are zero or one: $$ y \rightarrow h_2^5y + f_3h_2x +
\frac{f_3(f_3 + h_1h_2 + f_4h_2^2) + f_2h_2^2}{h_2^3}, \quad x\rightarrow
h_2^2x + f_4.$$ Dividing the equation by $h_2^{10}$ results in $h_2 =1$ and $f_4
= f_3 = f_2 = 0$. This equation of the curve is assumed when counting operations
in Section~\ref{sec:ramFieldCosts} over base field $k$ with char($k) = 2$. Note
that $\deg h = 2$ is required: the constant above is the only place the
coefficient $a_0$ of the substitution enters, and it does so multiplied by
$h_2$, so $f_2$ cannot be cleared when $h_2 = 0$. 
...
\begin{align*}
    v_{n_1} &= s_1(u_{n_1}s_0''+ u_{n_1}u_{11} - u_{n_1}^2 - s_0''u_{11}
    + u_{n_0} - u_{10}) - v_{11} - h_1 + h_2u_{n_1}\\
            &= s_1((u_{n_1} - s_0'')(u_{11} - u_{n_1}) + u_{n_0} - u_{10}) - v_{11} - h_1 + h_2u_{n_1}\\
    v_{n_0} &= s_1(u_{n_0}s_0'' + u_{11}u_{n_0} - u_{n_0}u_{n_1} -s_0''u_{10}) - v_{10} - h_0 + h_2u_{n_0}\\
            &= s_1(s_0''(u_{n_0} - u_{10}) + u_{n_0}(u_{11} - u_{n_1})) - v_{10} - h_0 + h_2u_{n_0}.
\end{align*} 
...
\begin{subroutine}[htbp]
    ...
    \Else 
        ...
        \Else 
            ...
            \Statex Go to Step~5 of Degree 2 Addition Algorithm~\ref{alg:explRAM2ADD}, with $d = d_{w_3}$.
        \EndIf
    \EndIf
    \end{algorithmic}
    \end{subroutine}
...
\Else 
    ...
    \Statex Go to Step~3 of Degree 1 and 2 Addition Algorithm~\ref{alg:explRAM12ADD}, with $d = d_w$.
\EndIf
\end{algorithmic}
\end{subroutine}
...
\hline
Degree 2&
22&4&42&2&  21&5&25&0&   21&4&24&0\TS\\
\hline
\hline
...
\hline
\textbf{A4 - This work}&
2&2&5&2& 21&4&24&0& 3&0&4&0& 8&1&19&0& 20&3&26&0\TS\\
...
by Step~21 of Balanced Doubling Algorithm~\ref{alg:g2balnudupl}, and $n_n = -1 + 3 -
\deg(u_n) = 2 - \deg(u_n)$ for Degree 1 Doubling with Up Adjust by Step~22 of
Balanced Doubling Algorithm~\ref{alg:g2balnudupl} (see Section~\ref{sec:expldbl}.) The basic
formulation for degree 1 doubling is described in
...
In Step~1 of degree 1 doubling with up adjustment
...
\Else 
    ...
    \Else 
        ...
        \State $s' = s_1'x + s_0' = d_{w_3}s_1x + d_{w_3}s_0 = a_{12}(v_2 - v_1) + m_3^3k) \pmod{u_2}$, with weight $d_{w_3}$.
        \Statex Go to line~5 of Degree 2 Addition Algorithm~\ref{alg:explSPLIT2ADD}, and let $d = d_{w_3}$.
    \EndIf
\EndIf
\end{algorithmic}
\end{subroutine}
...
\State $d = u_1 \pmod{u_2}$
\If{$d$ is 0}
    \Statex Go to Degree 1 and 2 Addition ($d = 0$) Subroutine~\ref{sub:explSPLIT12ADDd}.
\EndIf
...
Step~1 of degree 1 and 2 addition with up adjust computes a negative reduced
normalization of $v_1$ to accommodate the up adjustment. Steps~2--4 in both
algorithms is identical to the ramified setting. Steps~5--13, are similar to the
ramified setting other than $z = k/c_3$ is computed via Technique~\ref{tq:8}.
Therefore, in Steps~15--17 the computation of the required monic $u_n = (s(\vt -
...
\Else 
    ...
    \If{$n_2 = 0$} 
        \Statex Go to line~7 of Degree 1 and 2 Addition with Up Adjust Algorithm~\ref{alg:explSPLIT12ADDUP} and let $d = d_w$.
    \Else 
        \Statex Go to line~6 of Degree 1 and 2 Addition Algorithm~\ref{alg:explSPLIT12ADD} and let $d = d_w$.
    \EndIf
\EndIf
\end{algorithmic}
\end{subroutine}
...
\textbf{This}
&30&2&44&8 &27&1&37&3 &14&2&26&3 &3&0&5&0  &6&0&17&3 &4&0&13&3 &6&0&17&3 &4&0&9&3   \TS\\
\hline
